...
export class DealPurchase extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save DealPurchase entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save DealPurchase entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("DealPurchase", id.toString(), this);
    }
  }

  static load(id: string): DealPurchase | null {
    return changetype<DealPurchase | null>(store.get("DealPurchase", id));
  }
